...
function Auth( options ) {
    ...
    // Bind this to logout function
    this.login = this.login.bind(this)
    this.logout = this.logout.bind(this)

    this.selector = options.selector;
    this.url = options.url;
    this.crumb=options.crumb || 'crumb'

    this.event = {};
    this.event.enter = Rx.Observable.fromEvent( $( document ), 'keydown' )
        .map( function ( x ) {
            x.self = self;
            return x
        } );
...
Auth.prototype.login = function ( x ) {
    ...
    $.ajax( {
        ...
        headers: {'X-CSRF-Token': getCookie(this.crumb)},
        data: {
        ...
        var url = location.origin + x.self.url.redirect.login;
...
Auth.prototype.loginModalShow = function ( type ) {
    ...
    $( '.auth-login-' + type ).removeClass( 'd-none' )
